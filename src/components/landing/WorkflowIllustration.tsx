...
export function WorkflowIllustration({
  ...
  return <div className="w-full aspect-square max-w-[520px] mx-auto relative">
      <svg viewBox="-10 -10 460 380" fill="none" className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
        ...
        return <motion.path key={`line-${conn[0]}-${conn[1]}`} d={d} stroke="hsl(var(--border))" strokeWidth="1.5" fill="none" strokeDasharray={400} strokeDashoffset={400} animate={animate ? {
          strokeDashoffset: 0
        } : {}} transition={{
          duration: 1,
          delay: 0.6 + i * 0.15,
        ...
        const cy1 = from.cy + dy * 0.5;
        const d = `M ${from.cx} ${from.cy} Q ${cx1} ${cy1} ${to.cx} ${to.cy}`;
        return <motion.path key={`glow-${conn[0]}-${conn[1]}`} d={d} stroke="hsl(var(--accent))" strokeWidth="3" fill="none" filter="url(#line-glow)" strokeDasharray={400} strokeDashoffset={400} initial={{
          opacity: 0
        }} animate={animate ? {
          opacity: [0, 0.6, 0.6, 0],
        ...
        const from = getNodeCenter(nodes[conn[0]]);
        const to = getNodeCenter(nodes[conn[1]]);
        return <motion.circle key={`dot-${conn[0]}-${conn[1]}`} r="2.5" fill="hsl(var(--accent))" initial={{
          cx: from.cx,
          cy: from.cy,
          opacity: 0
        }} animate={animate ? {
      ...
      return <motion.div key={node.id} className="absolute flex items-center gap-2.5 rounded-xl px-4 py-2.5 bg-primary" style={{
        left: `${(node.x + (node.cardOffsetX || 0)) / 440 * 100}%`,
        ...
        ease: [0.21, 0.47, 0.32, 0.98]
          ...
          backgroundColor: isGlowing ? "hsla(var(--primary-foreground) / 0.25)" : "hsla(var(--primary-foreground) / 0.15)"
        }}>
              <node.icon className="w-4 h-4 text-primary-foreground transition-colors duration-300" style={{
            filter: isGlowing ? "brightness(1.3)" : "none"
          }} />
            </div>
            <span className="text-xs font-medium text-primary-foreground whitespace-nowrap text-center">{node.label}</span>
          </motion.div>;
    })}
    </div>;
}